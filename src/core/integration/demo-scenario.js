/**
 * BUMBA Demo Scenario
 * Demonstrates the full integrated system in action
 */

const { logger } = require('../logging/bumba-logger');
const { BUMBA } = require('./consciousness-integration-layer');
const { SystemHealthDashboard } = require('./system-health-dashboard');

class BumbaDemoScenario {
  constructor() {
...
class InteractiveDemo {
  constructor() {
    this.scenario = new BumbaDemoScenario();
    this.consciousness = null;
  }
  ...
  async start() {
    ...
    console.log('  start    - Run full demo scenario');
    console.log('  awaken   - Just awaken BUMBA');
    console.log('  teach    - Teach BUMBA with custom experience');
    console.log('  ask      - Ask BUMBA for wisdom');
    console.log('  status   - Show current state');
    console.log('  dashboard- Launch health dashboard');
    console.log('  exit     - Exit demo\n');

    const prompt = () => {
      rl.question('BUMBA> ', async (command) => {
        try {
    ...
    switch (command) {
      ...
      case 'awaken':
        if (!this.consciousness) {
          ...
          console.log('BUMBA is already awake.');
        }
        break;
        
      case 'teach':
        if (!this.consciousness) {
          console.log('Please awaken BUMBA first.');
          break;
        }
        // Simple teaching interface
        console.log('Teaching BUMBA... (simplified demo)');
        await this.consciousness.recordExperience({
          type: 'user_teaching',
          context: { source: 'interactive' },
          action: { type: 'learn' },
          outcome: { type: 'knowledge_gained' },
          success: true
        });
        console.log('🏁 BUMBA learned from your teaching!');
        break;
      ...
      case 'ask':
        ...
        const wisdom = await BUMBA.shareWisdom({ type: 'general' });
        console.log(`\n🟢 BUMBA says: ${wisdom.wisdom}\n`);
        break;
        
      case 'status':
        if (!this.consciousness) {
          console.log('BUMBA is sleeping.');
        } else {
          const state = BUMBA.getState();
          console.log('\nBUMBA Status:');
          console.log(`  Health: ${state.state.health}`);
          console.log(`  Experiences: ${state.metrics.experiences}`);
          console.log(`  Uptime: ${this.scenario.formatUptime(state.uptime)}\n`);
        }
        break;
      ...
      case 'dashboard':
        ...
        console.log('Launching dashboard... (Press Q to return)');
        const dashboard = new SystemHealthDashboard(this.consciousness);
...
module.exports = {
  ...
  // Quick demo runner
  runDemo: async () => {
    const demo = new BumbaDemoScenario();
    await demo.run();
  },
  
  // Interactive mode
  runInteractive: async () => {
    const interactive = new InteractiveDemo();
